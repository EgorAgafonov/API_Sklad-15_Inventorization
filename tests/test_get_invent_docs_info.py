...
class TestInventorizationSklad15:
    ...
    def test_select_data_from_invent_docs(self, query_value="?$select=id,name"):
        """Проверка GET-запроса функции 'Авторизация' на предмет предоставления сведений о id-номере и name-наименовании
    ...
    def test_get_strict_nums_of_invent_docs(self, query_value="?$top=5&$skip=16"):
