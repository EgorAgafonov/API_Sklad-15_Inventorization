...
class TestInventorizationSklad15:
    ...
    def test_select_data_from_docs_positive(self, query_value="?$select=id,name"):
        """Проверка GET-запроса функции 'Авторизация' на предмет предоставления сведений о id-номере и наименовании
        (name) документа инвентаризации. Строка параметра query использует верифицированные значения, указанные в
        документации на Swagger. Валидация теста успешна, если тело ответа содержит сведения о всех запрошенных
        атрибутах документов."""
